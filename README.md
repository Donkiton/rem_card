...
Версия сначала подготавливается отдельным коммитом и попадает в GitHub `main`.
Менеджер релизов считывает из GitHub точные версию и коммит, безопасно
синхронизирует чистую локальную ветку `main` и запускает:
...
Скрипт не изменяет версию, changelog или release-info, не создаёт коммит и не
выполняет `git push`. Он проверяет соответствие уже опубликованному исходному
коммиту, выполняет обязательные проверки, собирает и проверяет все EXE и
помещает пакет в локальный тестовый `UPD`. Временные `build` и `dist` после
обычной релизной сборки очищаются.
