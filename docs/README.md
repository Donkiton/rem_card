...
Обновлено: 2026-09-23. Обзор возможностей текущего `main` и быстрый запуск — в [README проекта](../README.md).

Интерфейс и состав поставки **5.1.0**: один `RemCard.exe` с четырьмя ролями и `RemCardUpdater.exe`. [Интерактивная карта архитектуры](https://donkiton.github.io/rem_card/codemap/codemap.html) обновлена для единого входа, допуска сессий и фоновой проверки базы.
