...
До этой команды локальный пакет обязан пройти приёмку на отдельной локальной/тестовой `Baza_rao3_jurnal`: обновление при запуске и после штатного закрытия, запуск всех ролей и изменённые рабочие сценарии. После создания `ready.ok` в сетевой production-базе релиз сразу доступен всем подключённым клиентам, поэтому production-публикация не используется как канареечный тест.
