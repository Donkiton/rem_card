...
class EmergencyLocalStore:
    ...
    def create_active_session_from_standby(
        ...
        *,
        ...
        source_error = validate_emergency_patient_source(standby_metadata.medical_db_path)
        if source_error or not standby_metadata.source_remote_db_path:
            raise EmergencyStoreError(source_error or "Не определена исходная база аварийной копии")
    ...
    def create_active_session_from_empty_database(
        ...
        *,
        ...
        raise EmergencyStoreError(
            "Создание пустой аварийной базы запрещено. Нужна проверенная копия с пациентами."
        )
    ...
    def mark_session_discarded(
        ...
        *,
        ...
        if metadata.status == "discarded":
            return metadata
        if metadata.merge_recovery_required:
            raise EmergencyStoreError("Сначала необходимо установить результат предыдущего переноса")
        if metadata.status not in {"active", "waiting", "merge_failed"}:
