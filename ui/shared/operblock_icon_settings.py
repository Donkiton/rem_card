...
def _load_icon_metadata_from_db(
    *,
    ...
    try:
        ...
        version, loaded_records = get_settings_service().get_operblock_icon_metadata_snapshot()
        loaded_records = dict(loaded_records or {})
    except Exception:
        with _METADATA_CONDITION:
    ...
    with _METADATA_CONDITION:
        if load_epoch != int(_ICON_REQUEST_EPOCH):
            ...
            records = _ICON_RECORD_CACHE.get("records")
            return _ICON_RECORD_CACHE.get("version"), dict(records) if isinstance(records, dict) else {}
        previous_version = _ICON_RECORD_CACHE.get("version")
        version_changed = previous_version is not None and previous_version != version
        if version_changed:
            _ICON_REQUEST_EPOCH += 1
        _ICON_RECORD_CACHE["version"] = version
        _ICON_RECORD_CACHE["checked_at"] = time.monotonic()
        _ICON_RECORD_CACHE["records"] = loaded_records
        _METADATA_LOADING = False
        _METADATA_CONDITION.notify_all()
    if version_changed:
        _BLOB_RECORD_CACHE.clear()
    return version, dict(loaded_records)
...
def _metadata_icon_record_from_records(normalized_keys: list[str], records: dict[str, dict]) -> dict | None:
    ...
    if label_slugs:
        for record in records.values():
            if not isinstance(record, dict) or not record.get("has_image_blob"):
                continue
    ...
    *,
    ...
    try:
        try:
            ...
            loaded_version, records = get_settings_service().get_operblock_icon_records([icon_key], include_blob=True)
        ...
        with _ICON_CACHE_LOCK:
            ...
            if current_version is not None and loaded_version != current_version:
                _ICON_RECORD_CACHE["checked_at"] = 0.0
                _ICON_REQUEST_EPOCH += 1
...
def _resolved_icon_record(
    ...
    *,
    ...
    return _load_blob_record(version, metadata, expected_epoch=expected_epoch)
...
def _prepare_operblock_icon_image(
    icon_keys,
    size: QSize | None,
    aspect_mode,
    transformation_mode,
...
def _finalize_operblock_icon_image(prepared) -> QPixmap:
...
def _scaled_pixmap(pixmap: QPixmap, target_size: QSize | None, aspect_mode, transformation_mode) -> QPixmap:
    if target_size is None or pixmap.isNull():
        return pixmap
    return pixmap.scaled(target_size, aspect_mode, transformation_mode)


def _load_file_pixmap(path: str, *, version, target_size, aspect_mode, transformation_mode) -> QPixmap:
    try:
        stat = os.stat(path)
    ...
    cache_key = (
        ...
        fingerprint,
        None if target_size is None else (target_size.width(), target_size.height()),
        _enum_cache_key(aspect_mode),
        _enum_cache_key(transformation_mode),
    )
    ...
    if cached is not None:
        return cached
    pixmap = QPixmap(path)
    pixmap = _scaled_pixmap(pixmap, target_size, aspect_mode, transformation_mode)
...
def request_operblock_icon_pixmap(
    receiver,
    icon_keys: str | Iterable[str],
    *,
    fallback_file: str = "",
    ...
    if not metadata_fresh or (has_db_icon and cached is None):
        normalized_keys = tuple(_normalized_icon_keys(icon_keys))
        request_key = (
            "operblock",
            request_epoch,
            normalized_keys,
        ...
        request_async_icon(
            receiver,
            request_key,
            lambda: _prepare_operblock_icon_image(
                normalized_keys,
                size,
                aspect_mode,
                transformation_mode,
            ...
            _finalize_operblock_icon_image,
            apply,
